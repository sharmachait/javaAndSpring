...
Validating controllers are controllers that validate a request before it is allowed to be processed 
...
the third part server that processes the admission request returns with and admissionReview object as well with the result of the processing and allowed status of true or false

can be configured by making a webhook configuration object on our k8s cluster