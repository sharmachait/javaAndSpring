...
1. /v1alpha1 - 
	...
	3. can be enabled by adding to the kube api server config at `/etc/kubernetes/manifests/kube-apiserver.yaml` as **--runtime-config=batch/v2alpha1** ina comma separated format
2. /v1beta1 - 
...
2. API objects must be able to round trip between API versions in a given release without information loss with the exception of some resources that may be dropped in some release
	...
	2. if v1alpha1 has a field in the manifest file, the future versions must necessarily have that field, they can not from it from their manifest, can only add new
